                             ...
                             Size& calibratedImageSize )
{
    ...
    cout << "intrinsicsFilename: " << filename << endl;
    cout << "width: " << calibratedImageSize.width << endl;
    cout << "height: " << calibratedImageSize.height << endl;
    cout << "cameraMatrix:\n " << cameraMatrix << endl;
    cout << "distCoeffs:\n " << distCoeffs << endl;
...
static bool readModelViews( const string& filename,
                            ...
                            vector<Vec6f>& poseList )
{
    ...
    for(FileNodeIterator it = all.begin(); it != all.end(); ++it)
    {
        FileNode n = *it;
    ...
    for (int i = 0; i < num_imgs; i++)
    {
        string img_file;
        fs >> img_file;
        imagelist.push_back(img_file);
        for (int j = 0; j < 9; j++) // skip K
            fs >> Kij;
        fs >> R.at<double>(0,0) >> R.at<double>(0,1) >> R.at<double>(0,2);
        fs >> R.at<double>(1,0) >> R.at<double>(1,1) >> R.at<double>(1,2);
        fs >> R.at<double>(2,0) >> R.at<double>(2,1) >> R.at<double>(2,2);
        Rodrigues(R, rvec);
        pose[0] = rvec.at<double>(0,0);
        pose[1] = rvec.at<double>(1,0);
        pose[2] = rvec.at<double>(2,0);
        fs >> pose[3] >> pose[4] >> pose[5];
        poseList.push_back(pose);
        cout << img_file << " " << pose << endl;
    }
#endif
    return true;
}
...
static void writeModelPLY(const string& modelFileName, const PointModel& model)
{
    fstream fs(modelFileName, ios_base::out);

    fs << "ply"
       << '\n' << "format ascii 1.0"
       << '\n' << "element vertex " << model.points.size()
       << '\n' << "property float x"
       << '\n' << "property float y"
       << '\n' << "property float z"
       << '\n' << "property uchar red"
                                ...
                                const Mat& cameraMatrix)
{
    ...
    else*/
    {
        ...
        Mat_<float> K1 = R1t*(iK*m1);
        Mat_<float> K2 = R2t*(iK*m2);
                          ...
                          PointModel& model )
{
    ...
    {
        Mat gray = imread(format("%s/%s", workSpace, imageList[i].c_str()),
    ...
    cout << alldescriptorsVec.size() << " " << descriptorSize << endl;
    Mat alldescriptors((int)alldescriptorsVec.size()/descriptorSize, descriptorSize,
                       CV_32F, &alldescriptorsVec[0]);
    ...
    for( size_t i = 0; i < nimages; i++ )
    {
        for( size_t j = i+1; j < nimages; j++ )
        {
            ...
            {
                // triangulate correspondent keypoints
                int i1 = pairwiseMatches[k][0];
                int i2 = pairwiseMatches[k][1];
                ...
                // project 3D 2D to calculate error
    ...
    printf("\nClustering same correspondences ");
    ...
    for( size_t i = 0; i < nimages; i++ )
    {
        img = imread(format("%s/templeSR%04d.png", workSpace, (int)i+1), 1);
...
int main(int argc, char** argv)
{
    ...
    Size calibratedImageSize;
    readCameraMatrix(format("%s/intrinsics.yml", workSpace),
                     cameraMatrix, distCoeffs, calibratedImageSize);

    Ptr<FeatureDetector> featureDetector = xfeatures2d::SurfFeatureDetector::create();
    Ptr<DescriptorExtractor> descriptorExtractor = xfeatures2d::SurfDescriptorExtractor::create();

    if(!readModelViews( format("%s/templeSR_par.txt", workSpace), imageList, poseList))
    {
    ...
    PointModel model;
    build3dmodel( featureDetector, descriptorExtractor,
                  workSpace, imageList, poseList, cameraMatrix,
                  model );

    printf("\nDone! Now saving the model ...\n");
    writeModel(format("%s/model.yml.gz", workSpace), "modelName", model);
    writeModelPLY(format("%s/model.ply", workSpace), model);
